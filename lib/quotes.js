...
export async function fetchQuote(symbol) {
  ...
  try {
    ...
    // Daily closes from the baseline day onward, for the sparklines and
    // the season race chart. The baseline day is the first point.
    const history = [];
    for (let i = baselineIdx; i < timestamps.length; i++) {
      if (closes[i] == null) continue;
      history.push({ date: toEasternDate(timestamps[i]), close: closes[i] });
    }
